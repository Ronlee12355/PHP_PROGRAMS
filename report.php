...
<html>
	...
	<body>
		...
		<?php
				...
				?>
				<div class="panel panel-default">
   <div class="panel-heading" style="text-align: center;"><strong style="font-size: 25px;">学术报告</strong></div>
      <ul class="list-group">
      	<?php
      		$sql="select * from `report` where 1 order by `istop` desc, `date` desc";
			//$DB->query($sql);
			$sarr=$DB->get_rows_array($sql);
//			var_dump($sarr);exit;
			foreach($sarr as $v){
				...
				if($v['istop']){
				?>
				<li class="list-group-item"><a href="report.php?id=<?php echo $v['id'];?>"><span style="color: red;">置顶</span><?php echo $titl;?></a></li>
			<?php	
				}else{
			?>
				<li class="list-group-item"><a href="report.php?id=<?php echo $v['id'];?>"><?php echo $titl;?></a></li>
		<?php
		}
			}
      		?>
