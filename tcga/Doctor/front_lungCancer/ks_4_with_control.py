...
def to_fd(case, control):
    """Calculate fd"""
    control = pd.read_csv(control, sep="\t", header=None, index_col=0)
    control = control.median(axis=1)
    case = read_file(case)
...
if __name__ == "__main__":
    ...
    case, control, outpath = sys.argv[1:]
    fd = to_fd(case, control) 
    rank = run_rank(fd)  # run generank
